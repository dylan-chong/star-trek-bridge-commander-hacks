...
def CreatePlayerShip(sShipClass, pSet, pcName, sWaypoint, bUnloadShip = 0):
	...
	if sShipClass == 'Galaxy' or sShipClass == 'SovereignR' or sShipClass == 'Sovereign':
		import time
...
###############################################################################
def StartCutscene(pAction, fTimeToComeIn = 1.0, fCoveredArea = 0.075, bHideReticle = 1, bSilenceCallOuts = 1, bStopTractor = 1):
...
###############################################################################
def GetShip(pcShipName, pSet = None, bAnySet = FALSE):	
	assert pcShipName
	if(pSet):
...
def SetupBridgeSet(pcSetName, pcModelName, fX = -40, fY = 65, fZ = -1.55, fMX = 0, fMY = 0, fMZ = 0):
	...
	if (pNiCamera is None):
		...
		if (pcModelName == "data/Models/Sets/DBridge/DBridge.nif"):								#GalaxySeated
			pCamera = App.CameraObjectClass_Create(-1, -10, 40, -1.55, 0, 0, 1, "maincamera")
		elif (pcModelName == "data/Models/Sets/EBridge/EBridge.nif"):							#SovereignSeated
			pCamera = App.CameraObjectClass_Create(0, 25, 65, -1.55, 0, 0, 1, "maincamera")
		elif (pcModelName == "data/Models/Sets/intrepidbridge/intrepidbridge.nif"):			#IntrepidSeated
			pCamera = App.CameraObjectClass_Create(-32, -50, 48, -1.55, 0, 0, 1, "maincamera")
		elif (pcModelName == "data/Models/Sets/defiantbridge/defiantbridge.nif"):				#DefiantSeated
			pCamera = App.CameraObjectClass_Create(-3, -75, 5, -1.57, 0, 0, 1, "maincamera")
		elif (pcModelName == "data/Models/Sets/FCExcelsiorBridge/ExcelsiorBridge.nif"):			#ExcelsiorSeated
			pCamera = App.CameraObjectClass_Create(8, -50, 50, -1.57, 0, 0, 1, "maincamera")
		elif (pcModelName == "data/Models/Sets/Klingon/BOPBridge.nif"):
			pCamera = App.CameraObjectClass_Create(0, -120, 45, -1.57, 0, 0, 1, "maincamera")
		elif (pcModelName == "data/Models/Sets/Klingon/BOPbridge.nif"):
			pCamera = App.CameraObjectClass_Create(0, -120, 45, -1.57, 0, 0, 1, "maincamera")
		elif (pcModelName == "data/Models/Sets/Romulan/romulanbridge.nif"):
			pCamera = App.CameraObjectClass_Create(0, -130, 65, -1.57, 0, 0, 1, "maincamera")
		elif (pcModelName == "data/Models/Sets/Cardassian/cardbridge.nif"):
			pCamera = App.CameraObjectClass_Create(6, -55, 58, -1.57, 0, 0, 1, "maincamera")
		else:
...
def GetPositionOrientationFromProperty(pObject, sPropertyName):
	...
	#debug("GetPositionOrientationFromProperty: Got %d properties" % pInstanceList.TGGetNumItems())
