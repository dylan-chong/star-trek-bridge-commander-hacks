...
def CreateShip(pcScript, pSet, pcIdentifier, pcLocationName, iWarpFlash = 0, bGrabPreloaded = 1, shipDef = None):
	...
	if not pShip:
		...
		pMass = pShip.GetShipProperty().GetMass()
		if pMass:
			pShip.SetSplashDamage(math.sqrt(pMass) * 70.0, math.sqrt(math.sqrt(math.sqrt(pMass))) * 30.0)
			#print pShip.GetSplashDamage()
			#print pShip.GetSplashDamageRadius()
