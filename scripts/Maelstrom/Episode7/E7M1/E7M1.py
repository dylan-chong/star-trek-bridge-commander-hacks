...
ET_CREATE_FEDS			= App.Mission_GetNextEventType()
...
def PreLoadAssets(pMission):
	...
	loadspacehelper.PreloadShip("KessokLight", 8)
	loadspacehelper.PreloadShip("Keldon", 7)
	loadspacehelper.PreloadShip("Galor", 5)
	loadspacehelper.PreloadShip("Hideki", 4)
...
#	DebugPrint("Creating characters and their sets")
	...
	MissionLib.SetupBridgeSet("ABridgeSet", "data/Models/Sets/FCExcelsiorBridge/ExcelsiorBridge.nif", -40, 65, -1.55)
	MissionLib.ReplaceBridgeTexture(None, "ABridgeSet", "MSD_wall.tga", "data/Models/Sets/FCExcelsiorBridge/MSD_wall_alt.tga")
	MissionLib.SetupCharacter("Bridge.Characters.MacCray", "ABridgeSet")
...
#	DebugPrint("Creating ships.\n")
	...
	pPlayer = MissionLib.CreatePlayerShip("SovereignR", pSet, "player", "Player Start")
	pStarbase = loadspacehelper.CreateShip( "FedStarbase", pSet, "Starbase 12", "Starbase Location" )

	#Damage the Player's Ship
	#pPlayer.DamageSystem(pPlayer.GetHull(), 6000)
	#pPlayerShields = pPlayer.GetShields()
	#pPlayerShields.SetCurShields(App.ShieldClass.FRONT_SHIELDS,4000)
	#pPlayerShields.SetCurShields(App.ShieldClass.LEFT_SHIELDS,5000)
	#pPlayerShields.SetCurShields(App.ShieldClass.RIGHT_SHIELDS,3000)
	#pPlayerShields.SetCurShields(App.ShieldClass.REAR_SHIELDS,6000)
	#pPlayerShields.SetCurShields(App.ShieldClass.TOP_SHIELDS,5000)
	#pPlayerShields.SetCurShields(App.ShieldClass.BOTTOM_SHIELDS,3000)
	...
	pEnemies.AddName("Hideki1")
	pEnemies.AddName("Hideki2")
	pEnemies.AddName("Hideki3")
	pEnemies.AddName("Hideki4")
	...
	MissionLib.SaveGame("E7M1A(Starbase12)-")

	MissionLib.SetTotalTorpsAtStarbase("7 PHOTON", -1)
	MissionLib.SetMaxTorpsForPlayer("7 PHOTON", 250)
...
#	DebugPrint("Set up sequence to add phased torpedoes next time you dock with Starbase 12")
	...
	pAction = App.TGScriptAction_Create(__name__, "FixTorpMax", "TRANSPHASIC", 3)
	pSequence.AppendAction(pAction)

	pAction = App.TGScriptAction_Create("MissionLib", "LoadTorpedoes", "TRANSPHASIC", 3)
	pSequence.AppendAction(pAction)
...
#		DebugPrint("MacCray warns you about hitting the Geronimo")
		...
		pABridgeSet = App.g_kSetManager.GetSet("ABridgeSet")
		pMacCray = App.CharacterClass_GetObject (pABridgeSet, "MacCray")
		...
		pAction	= App.TGScriptAction_Create("MissionLib", "ViewscreenOn", "ABridgeSet", "MacCray")
		pSequence.AppendAction(pAction)
...
#	DebugPrint("Creating second wave of Cardassians")	
	...
	# Create ships
	pKeldon4 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon4", "Freighter 1 Start", 1 )
	pKeldon5 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon5", "Freighter 2 Start", 1 )
	pKeldon6 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon6", "Freighter 3 Start", 1 )
	pGalor1 = loadspacehelper.CreateShip( "Galor", pSet, "Galor1", "Freighter 4 Start", 1 )
	pGalor2 = loadspacehelper.CreateShip( "Galor", pSet, "Galor2", "Freighter 5 Start", 1 )
	pHideki1 = loadspacehelper.CreateShip( "Hideki", pSet, "Hideki1", "Freighter 6 Start", 1 )
	...
	pGalor1.SetAI(KeldonAI.CreateAI(pGalor1))

	iNum = App.g_kSystemWrapper.GetRandomNumber(2)
	if (iNum == 1):
		pGalor2.SetAI(KeldonAI.CreateAI(pGalor2))
	else:
		pGalor2.SetAI(Keldon2AI.CreateAI(pGalor2))

	pHideki1.SetAI(KeldonAI.CreateAI(pHideki1))
...
#	DebugPrint("Creating third wave of Cardassians")	
	...
	# Create ships
	pKessok1 = loadspacehelper.CreateShip( "KessokLight", pSet, "Kessok1", "Attacker 1 Start", 1 )
	pKessok2 = loadspacehelper.CreateShip( "KessokLight", pSet, "Kessok2", "Attacker 2 Start", 1 )
	pKessok3 = loadspacehelper.CreateShip( "KessokLight", pSet, "Kessok3", "Attacker 3 Start", 1 )
...
#	CreateFourthWave()
#
#	Create the fourth wave of Cardassian ships
#
#	Args:	pObject	- TGObject object
#			pEvent	- TGEvent object
#
...
def CreateFourthWave(pObject, pEvent):
	...
	if (iMissionState == MISS_LOST):
#		DebugPrint("Bypassing Fourth Wave")
		return	
...
#	DebugPrint("Creating Fourth wave of Cardassians")	

	pSet = App.g_kSetManager.GetSet("Starbase12")

	# Create ships
	pGalor3 = loadspacehelper.CreateShip( "Galor", pSet, "Galor3", "Freighter 1 Start", 1 )
	pHideki2 = loadspacehelper.CreateShip( "Hideki", pSet, "Hideki2", "Freighter 2 Start", 1 )
	pHideki3 = loadspacehelper.CreateShip( "Hideki", pSet, "Hideki3", "Freighter 3 Start", 1 )
	pKessok4 = loadspacehelper.CreateShip( "KessokLight", pSet, "Kessok4", "Freighter 4 Start", 1 )
	pKessok5 = loadspacehelper.CreateShip( "KessokLight", pSet, "Kessok5", "Freighter 5 Start", 1 )
	pKessok6 = loadspacehelper.CreateShip( "KessokLight", pSet, "Kessok6", "Freighter 6 Start", 1 )
	...
	pGalor3.SetAI(KeldonAI.CreateAI(pGalor3))
	pHideki2.SetAI(KeldonAI.CreateAI(pHideki2))
	pHideki3.SetAI(KeldonAI.CreateAI(pHideki3))
	pKessok4.SetAI(Kessok1AI.CreateAI(pKessok4))
	pKessok5.SetAI(Kessok1AI.CreateAI(pKessok5))
	pKessok6.SetAI(Kessok1AI.CreateAI(pKessok6))

	g_pFelix.SayLine(pMissionDatabase, "Incoming6", "Captain", 1)
...
def CreateFifthWave(pObject, pEvent):
	...
	if (iMissionState == MISS_LOST):
#		DebugPrint("Bypassing Fifth Wave")
		return	
...
#	DebugPrint("Creating Fourth wave of Cardassians")	
	...
	# Create ships
	pKeldon7 = loadspacehelper.CreateShip( "Keldon", pSet, "Keldon7", "Freighter 1 Start", 1 )
	pGalor4 = loadspacehelper.CreateShip( "Galor", pSet, "Galor4", "Freighter 2 Start", 1 )
	pGalor5 = loadspacehelper.CreateShip( "Galor", pSet, "Galor5", "Freighter 3 Start", 1 )
	pHideki4 = loadspacehelper.CreateShip( "Hideki", pSet, "Hideki4", "Freighter 4 Start", 1 )
	pKessok7 = loadspacehelper.CreateShip( "KessokLight", pSet, "Kessok7", "Freighter 5 Start", 1 )
	pKessok8 = loadspacehelper.CreateShip( "KessokLight", pSet, "Kessok8", "Freighter 6 Start", 1 )
	...
	pGalor4.SetAI(KeldonAI.CreateAI(pGalor4))
	pGalor5.SetAI(KeldonAI.CreateAI(pGalor5))
	pHideki4.SetAI(KeldonAI.CreateAI(pHideki4))
	pKessok7.SetAI(Kessok1AI.CreateAI(pKessok7))
	pKessok8.SetAI(Kessok1AI.CreateAI(pKessok8))
...
#	CreateFeds()
#
#	Create the Federation Reinforcments and bring them into the fray
#
#	Args:	pObject	- TGObject object
#			pEvent	- TGEvent object
#
#	Return:	none
###############################################################################
def CreateFeds(pObject, pEvent):
#	DebugPrint("Creating Feds\n")	

	#Get the set
	pSet = App.g_kSetManager.GetSet("Starbase12")

	#Create the Allies
	pTitan = loadspacehelper.CreateShip( "Luna", pSet, "USS Titan", "Akira Start", 1 )
	pExcalibur = loadspacehelper.CreateShip( "Ambassador", pSet, "USS Excalibur", "Nebula Start", 1 )

	global bFedsArrive, bAreaScanned
	bFedsArrive = TRUE
	bAreaScanned = FALSE

#	DebugPrint ("Making the Feds's Warp and Impulse Engines invincible.")
	pWarp = pExcalibur.GetWarpEngineSubsystem()
	if (pWarp):
		MissionLib.MakeSubsystemsInvincible(pWarp)
	pImpulse = pExcalibur.GetImpulseEngineSubsystem()
	if (pImpulse):
		MissionLib.MakeSubsystemsInvincible(pImpulse)

	pWarp = pTitan.GetWarpEngineSubsystem()
	if (pWarp):
		MissionLib.MakeSubsystemsInvincible(pWarp)
	pImpulse = pTitan.GetImpulseEngineSubsystem()
	if (pImpulse):
		MissionLib.MakeSubsystemsInvincible(pImpulse)


	pExcalibur.SetAI( FriendlyAI.CreateAI(pExcalibur) )
	pTitan.SetAI( FriendlyAI.CreateAI(pTitan) )
...
#	DebugPrint("Adding GoToStarbase12, Starbase12Survive and RepelCardassians Goals.\n")
	...
	# Start a timer to create the Federation reinforcments
	MissionLib.CreateTimer(ET_CREATE_FEDS, __name__ + ".CreateFeds", fStartTime + 390, 0, 0)

	# Create a Timer that triggers the CreateFourthWave Function
	MissionLib.CreateTimer(ET_FOURTH_WAVE, __name__ + ".CreateFourthWave", fStartTime + 405, 0, 0)

	# Create a Timer that triggers the CreateFourthWave Function
	MissionLib.CreateTimer(ET_FIFTH_WAVE, __name__ + ".CreateFifthWave", fStartTime + 450, 0, 0)
...
def WarpAfterKessoks(pAction):
	...
	pExcalibur = App.ShipClass_GetObject(pSet, "USS Excalibur")
	pTitan = App.ShipClass_GetObject(pSet, "USS Titan")
...
#		DebugPrint("MacCray warps after the fleeing enemies")
	...
	if(pExcalibur):
		pExcalibur.SetAI( WarpAI.CreateAI(pExcalibur, 1) )

	if(pTitan):
		pTitan.SetAI( WarpAI.CreateAI(pTitan, 1) )
...
#	DebugPrint("Attack repelled...")
	...
	MissionLib.SaveGame("E7M1B(Starbase12)-")
...
#					DebugPrint("Geronimo hull down to 25 percent")
					...
					pAction = App.CharacterAction_Create(g_pKiska, App.CharacterAction.AT_SAY_LINE, "E7M1GeronimoArrive3", None, 0, pMissionDatabase)
					pSequence.AppendAction(pAction)
					pAction	= App.TGScriptAction_Create("MissionLib", "ViewscreenOn", "EAridgeSet", "MacCray")
					pSequence.AppendAction(pAction)
...
#					DebugPrint("Starbase 12 hull down to 50 percent")
					...
					pSet = App.g_kSetManager.GetSet("Starbase12")
					pEnt_E = App.ShipClass_GetObject(pSet, "USS Enterprise")
					if(pEnt_E) and not (iShipsDefeated == 16):
