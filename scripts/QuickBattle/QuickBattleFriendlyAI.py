...
def CreateAI(pShip):
	...
	pAttack = AI.Compound.BasicAttack.CreateAI(pShip, App.ObjectGroup_FromModule("QuickBattle.QuickBattle", "pEnemies"), Difficulty = QuickBattle.GetCurrentAILevel(), FollowTargetThroughWarp=1, UseCloaking=1, AvoidTorps = 0)
...
def CreateDroneAI(pShip, groupToAttack):
	#########################################
	# Creating CompoundAI Attack at (194, 57)
	import AI.Compound.BasicAttack
	pAttack = AI.Compound.BasicAttack.CreateAI(pShip, groupToAttack, Difficulty = QuickBattle.GetCurrentAILevel(), FollowTargetThroughWarp=1, UseCloaking=1, AvoidTorps = 0)
	# Done creating CompoundAI Attack
	#########################################
	#########################################
	# Creating ConditionalAI Wait at (83, 155)
	## Conditions:
	#### Condition TimePassed
	pTimePassed = App.ConditionScript_Create("Conditions.ConditionTimer", "ConditionTimer", 0 * 7, 0)
	## Evaluation function:
	def EvalFunc(bTimePassed):
		ACTIVE = App.ArtificialIntelligence.US_ACTIVE
		DORMANT = App.ArtificialIntelligence.US_DORMANT
